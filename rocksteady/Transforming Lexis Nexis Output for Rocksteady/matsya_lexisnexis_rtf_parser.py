...
TEMPLATE_FILE = r"/home/sam/dev/fyp/rocksteady/Transforming Lexis Nexis Output for Rocksteady/lexis_rft_template.txt"
DEBUG = False
...
class ArgParser:
    ...
    def parseArguments(self):
        ...
        parser.add_argument(
            ...
            "-i",
            help=
            "Input Directory with the Lexis Nexis RTF file (*** No formatting ***)",
            required=True)
        parser.add_argument(
            "--outputFile",
            "-o",
            help="Output file to write the lexis nexis parsed txt file",
            required=True)
        parser.add_argument('--debug',
                            '-d',
                            help="Enable Debug (** Will flood the screen **)",
                            action='store_true',
                            required=False)
...
class Utils:
    ...
    def createDirIdenpotent(self, filename):
        try:
            LOGGER.debug(
                f'Creating the directory for the filename {filename} if necessary'
            )
...
class ReadInputFile:
    ...
    def readFileContents(self):
        ...
        try:
            ...
            LOGGER.error(
                f'Error in reading the file {self.filename}. Error = {e}')
            return None
...
class WriteOutputFile:
    def __init__(self, outputFilePath):
        LOGGER.debug(
            f'Init Output writer with outputfilePath = {outputFilePath}')
    ...
    def writeOutput(self, text):
        try:
            ...
            with open(self.outputFilePath, 'a+',
                      encoding='utf8') as writeFileObj:
                writeFileObj.write(text + '\n')
            LOGGER.info(
                f'Completed Writing output to the file {self.outputFilePath}')
        except Exception as e:
            LOGGER.error(
                f'Error in writing to the file {self.outputFilePath}. Error = {e}'
            )
...
def readFiles(inputDir):
    files = [
        join(inputDir, f) for f in listdir(inputDir)
...
def main():
    print(
        f"{datetime.now().strftime('%B %d, %Y %A %I:%M %p GMT')}: *** Parsing Started ***"
    )
    parseCLI()
    ...
    print(
        f"{datetime.now().strftime('%B %d, %Y %A %I:%M %p GMT')}: *** Parsing Completed ***"
    )
